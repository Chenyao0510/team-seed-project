...
- ユーザーが AI 同士の議論に**リアルタイムで介入**（反論・質問・観点追加）できる UI を体験の中心に据える
- 介入によって議論の流れが目に見えて変わる演出を入れる
- 結論画面では「ユーザーの介入が議論に与えた影響」を称賛する文言を入れる

### データフロー

- フロントとバックは**単一の State オブジェクト (JSON)** を POST のリクエスト/レスポンスでキャッチボールする
- State オブジェクトが**唯一の信頼できる情報源 (single source of truth)**
- スキーマは `DECISIONS.md` の D01 に従う
