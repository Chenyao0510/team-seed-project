...
```json
{
  ...
  "characters": [
    {"name": "string", "avatar_url": "string", "gender": "male | female | robot | null", "persona": "string (省略可)"}
  ],
  "chat_history": [
...
- `DECISIONS.md`: D01 `CharacterRef` スキーマに `gender` を追加
...
- `backend/app/models.py`: `Gender`, `CharacterRef.gender`, `AddCharacterResponse.gender`
- `backend/app/gemini_client.py`: `classify_gender(name: str) -> Gender`
- `backend/app/routes.py`: `add_character` で `classify_gender` を呼ぶ。
  `/api/tts` に `gender` クエリパラメータを追加
- `backend/app/tts.py`: `SPEAKER_POOLS` 定義
- `frontend/src/types/state.ts`: `Character.gender`
...
- フロント prefetch だけだと初回ターン・ユーザー介入直後・ユーザーがすぐ「次へ」を
  押した場合に miss する。サーバーキャッシュを足すと、prefetch が
  間に合わなかったケースでも 2 回目以降のリクエストが即返る
...
- `functools.lru_cache`: 非同期関数に直接かけられない。 OrderedDic 直書きを採用。
- Redis / SQLite: D03（DB 入れない）に反する。プロセス内 LRU で十分。

**影響範囲**:

- `docs/ARCHITECTURE.md`: `/api/tts` 行に LRU + coalescing を追記
- `backend/app/tts.py`: `_tts_cache`, `_inflight` 等の実装

---

## D19: 人物ペルソナ (`persona`) を導入し、発言生成プロンプトを断定的に改訂する (T62/T64)
...
- persona が設定されている場合はプロンプト冒頭で提示し、その人物像・口調・専門・経験に
  基づいて語らせる。
- 同調の枕詞や過度な敬意を避け、反対意見には直接反論する。
- 自分の過去発言と矛盾しない立場を保ち、テーマ・現在の論点から逸脱しない（T64）。

**理由**:

- キャラクターは従来 `name` のみが LLM に渡り、「本人らしさ」はモデル自身の名前知識に
  依存していた。無名・カスタム人物では機能せず、また既存プロンプトは紳士的・冗長だった。
- `persona` を1〜2文の短い構造化フィールドにすることで、D04 の JSON Mode 制約内に収まる。
...
- `frontend/src/types/state.ts`: `persona?: string`
- `fixtures/debate_state_sample.json`: 既存キャラに persona 例を追加

---

## D20: 発言を hook/body/reasoning_target/concepts に構造化し、タイプライター演出する (T72)

**判断**: AI 人物の発言出力を単一文字列から構造化スキーマ
`{ hook, body, reasoning_target, concepts }` に変える。

- `hook`: 最初に表示する短い反応の一句（〜15文字）。
- `body`: hook に続く主張本体。`hook + body` 合計で**日本語60文字以内**。
- `reasoning_target`: 反応している直前発言を「話者名: 要点」で短く。
- `concepts`: body 内に実際に登場する重要語を1〜2個。フロントがその部分文字列を強調表示する。

`DebateState.current_speech` は廃止せず、`hook + body` をバックエンドで合成した**導出値**として
温存する（TTS・archive 等の互換維持）。表示専用に `current_hook / current_body / current_reasoning_target / current_concepts` を追加。

フロントは hook を即時表示し、body をタイプライターで描画、concepts を
ハイライト、reasoning_target をチップ表示する。

発言生成プロンプトは10の鉄則に改訂。persona は D19 で導入したが、
本判断で役割を **口調・立場・専門領域の色付け専用** に限定し、persona の朗読・自己紹介を禁止する。

**追補 (2026-06-14)**: 初期投入後、出力に「その人物の経歴・専門領域に基づいた具体例」が
現れにくい課題が見つかったため、鉄則6を改訂し、自分自身の経歴・専門領域に根ざした経験談
を必須化した。

**理由**:

- D19（persona 注入）の副作用で、発言が「経歴を朗読する講釈」化するのを防ぎ、
  "一緒に考えている" 体験（reacting / questioning）を強制するため。
- hook を即時表示 + body をタイプライターにすることで演出を強化。

**影響範囲**:

- `backend/app/models.py`: `AgentThoughtOutput`, `DebateState`
- `backend/app/debate.py`: `_compose_speech`
- `backend/app/gemini_client.py`: `_speech_rules` 新設 + プロンプト改訂
...
新しい判断は `D21`, `D22`, ... と連番で追加し、`判断 / 理由 / 影響範囲` を書く。
