...
```json
{
  ...
  "current_speech": "string (hook + body の合成。導出値)",
  "current_hook": "string (省略可)",
  "current_body": "string (省略可)",
  "current_reasoning_target": "string (省略可)",
  "current_concepts": ["string"],
  "current_points": ["string"],
  "characters": [
    {"name": "string", "avatar_url": "string", "persona": "string (省略可)"}
  ],
  "chat_history": [
...
`current_speech` は `current_hook + current_body` をバックエンドで合成した導出値で、TTS と `chat_history` への archive はこれを読む（D18）。`current_hook` / `current_body` / `current_reasoning_target` / `current_concepts` は Screen 1 のタイプライター演出のための表示専用フィールド。`characters` はステージ上に並ぶ参加者 roster。`active_character` はこの roster のいずれかの `name` を指す。`chat_history` は過去発話の追記ログで、Setup 直後は空配列。`turn_count` は `/api/next_turn` が呼ばれるたびにバックエンドが+1する整数で、Reflection Turn (T26/T27) の発火判定に使う。`user` はステージ右端に固定表示されるユーザー自身の表示情報で、`name` は介入発言の話者名（既定 `あなた`）、`avatar_url` は Screen 0 で登録したアバター。
...
**スキーマ進化メモ (T62 / D17)**: 各キャラクターの「人物像・口調・専門・価値観」を1〜2文で表す `persona` を `characters[]` に追加。発言生成プロンプトに注入し、本人らしさ・断定的な口調を強化する。後方互換のため optional（既定 `""`）。
...
## D17: 人物ペルソナ (`persona`) を導入し、発言生成プロンプトを断定的に改訂する (T62/T64)

**判断**: `CharacterRef` に `persona: str = ""`（人物像・口調・専門・価値観を1〜2文）を
追加する。テンプレート8人は事前に手書きの persona を持たせ、自由入力で追加する
カスタムキャラクターは `/api/add_character` 時に Gemini で best-effort 生成する
（失敗時は `""` のままでよい）。

合わせて `_build_agent_thought_prompt` / `_build_next_turn_prompt`
(`backend/app/gemini_client.py`) の発言生成ルールを改訂する:

- `current_speech` は「1〜2文を基本、最大3文」とし、前置き・両論併記・冗長な説明を避け
  短く断定的に述べる。
- persona が設定されている場合はプロンプト冒頭で提示し、その人物像・口調・専門・経験に
  基づいて語らせる（T62: 本人らしさ）。
- 同調の枕詞や過度な敬意を避け、反対意見には直接反論する（T62）。
- 自分の過去発言と矛盾しない立場を保ち、テーマ・現在の論点から逸脱しない、既出の主張を
  そのまま繰り返さず議論を一歩進める（T64: 一貫性・論点ズレ防止・堂々巡り防止）。

**理由**:

- キャラクターは従来 `name` のみが LLM に渡り、「本人らしさ」はモデル自身の名前知識に
  依存していた。無名・カスタム人物では機能せず、また既存プロンプトは紳士的・冗長で
  AI同士が両論併記しがちだった。
- `persona` を1〜2文の短い構造化フィールドにすることで、D04 の JSON Mode 制約内に収まり、
  既存の `responseSchema` パイプラインを変更せずに済む。
- 既定値 `""` のため既存 State / fixtures は無改修で valid。persona が空の場合は
  プロンプト側でモデル自身の知識に委ねるフォールバック文言にする。

**影響範囲**:

- `backend/app/models.py`: `CharacterRef.persona` / `AddCharacterResponse.persona` /
  新規 `CharacterPersonaOutput`
- `backend/app/character_templates.py`: `_TEMPLATE_CATALOG` に persona を追加
- `backend/app/gemini_client.py`: `generate_character_persona` 新設 + プロンプト改訂
- `backend/app/routes.py`: `/api/add_character` でペルソナ生成を best-effort 実行
- `frontend/src/types/state.ts` / `frontend/src/api/client.ts`: `persona?: string`
- `frontend/src/screens/SetupScreen.tsx` / `CharacterTemplatePanel.tsx` /
  `frontend/src/lib/buildDebateState.ts`: persona をスレッド
- `fixtures/debate_state_sample.json`: 既存キャラに persona 例を追加

---

## D18: 発言を hook/body/reasoning_target/concepts に構造化し、タイプライター演出する (T69)

**判断**: AI 人物の発言出力を単一文字列から構造化スキーマ
`{ hook, body, reasoning_target, concepts }` に変える。

- `hook`: 最初に表示する短い反応の一句（〜15文字）。直前の特定発言への即座のリアクション。
- `body`: hook に続く主張本体。`hook + body` 合計で**日本語60文字以内**。
- `reasoning_target`: 反応している直前発言を「話者名: 要点」で短く。UI で「→ 相手へ」の
  返信チップとして表示する。
- `concepts`: body 内に実際に登場する重要語を1〜2個。フロントがその部分文字列を強調表示する。

`DebateState.current_speech` は廃止せず、`hook + body` をバックエンドで合成した**導出値**として
温存する（TTS `/api/tts`・`chat_history` archive・フロントのオートシンク発火条件が依存）。
表示専用に `current_hook / current_body / current_reasoning_target / current_concepts` を追加。

フロントは hook を即時表示し、300〜600ms 後に body をタイプライターで描画、concepts を
ハイライト、reasoning_target をチップ表示する。

発言生成プロンプトは10の鉄則（直前発言へ反応／思想を説明しない／自己紹介しない／名言を
繰り返さない／新しい角度を1つ足す／具体例・歴史・思考実験・実務帰結のいずれかを含む／
60文字以内／問いを優先／緊張を優先／講釈でなく参加者として）に改訂。persona は D17 で導入したが、
本判断で役割を **口調・立場・専門領域の色付け専用** に限定し、persona の朗読・自己紹介を禁止する。

**追補 (2026-06-14)**: 初期投入後、出力に「その人物の経歴・専門領域に基づいた具体例」が
ほとんど現れず、議論に参加・応答している実感が薄いという課題が見つかった。
- 鉄則6を改訂: 具体例 / 歴史的事件 / 思考実験 / 実務的な帰結は、**自分自身の経歴・専門領域に
  根ざした経験談**として語ることを必須化（一般論・教科書的な例を禁止）。
- 鉄則1を補強: body の冒頭付近で相手の発言に直接触れる応答的な言葉（「それは」「でも」
  「だったら」等）を使い、議論への参加・返答であることを明示させる。
- `_persona_clause` も「具体例を出すときはこの経歴・専門領域に根ざした自分自身の経験や視点
  から選ぶこと」を明記するよう更新。

**理由**:

- D17（persona 注入 + 断定ルール）の副作用で、発言が「経歴・信条を朗読する講釈」化し、
  "一緒に考えている" 体験が薄れた。reacting / questioning / advancing を強制する必要があった。
- hook を即時表示 + body をタイプライターにすることで「今まさに考えて言葉にしている」感を演出。
- `current_speech` を導出値として残すことで、TTS・archive・オートシンクの既存配線を壊さずに
  追加実装できる（後方互換）。

**影響範囲**:

- `backend/app/models.py`: `AgentThoughtOutput` / `NextTurnLLMOutput` / `DebateState`
- `backend/app/debate.py`: `_compose_speech` + `advance_turn` / `generate_thoughts`
- `backend/app/gemini_client.py`: `_speech_rules` 新設 + 両プロンプト改訂 + `_persona_clause` 再定義
- `frontend/src/types/state.ts` / `frontend/src/lib/buildDebateState.ts`
- `frontend/src/components/debate/Typewriter.tsx`（新規）
- `frontend/src/screens/DebateStage.tsx`: `TelopBox` 描画 + 配線 + `submitIntervention`
- `fixtures/debate_state_sample.json`、各 backend テスト
