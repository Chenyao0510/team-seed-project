...
def _persona_clause(state: DebateState, character_name: str) -> str:
    ...
    persona は **口調・立場・思考の進め方の参考にのみ** 使う。自己紹介・思想の説明・
    persona の朗読は禁止。「らしさ」は、このキャラクターが好んで語る結論や持説テーマに
    話を引き寄せることではなく、このキャラクターらしい考え方・問いの立て方・検証の仕方を
    今の論点に適用することで表現する（例: 哲学者なら、どんな話題でも自分の得意な抽象的
    テーマに引き戻すのではなく、相手の前提を執拗に問い直す、という「方法」のほうを借りる）。
    persona が未設定の場合は、モデル自身の知識から口調と方法だけを借りるよう促す。
    """
    character = next((c for c in state.characters if c.name == character_name), None)
    persona = character.persona if character else ""
    if persona:
        return (
            "あなたの人物像（口調・立場・専門領域、および考え方・問いの立て方の参考にのみ"
            "使う。説明・朗読はしない。今の論点に関係ない持説テーマに話を引き戻さず、"
            "この人物らしい『考え方・検証の仕方』を今の論点に適用すること。"
            "具体例を出すときは、この経歴・専門領域に根ざした自分自身の経験や視点から"
            f"選ぶこと）: {persona}\n"
        )
    return (
        f"「{character_name}」本人の口調・立場・専門領域、および考え方・問いの立て方を"
        "あなたの知識から借りて話す（経歴の説明や自己紹介はしない。今の論点に関係ない"
        "持説テーマに話を引き戻さず、具体例はその専門領域に根ざした自分自身の経験や"
        "視点から選ぶこと）。\n"
    )
...
def _speech_rules(theme: str) -> str:
    ...
    「直前の発言への反応」にし、新しい角度・80文字以内・断定と緊張を優先させる。
    """
    return (
        "発言は reasoning_target / focus_point / move_type / hook / body / concepts の順に"
        "確定させること:\n"
        "- reasoning_target: 今あなたが反応する直前の発言を「話者名: 検証可能な主張」の形で"
        "一文に特定する。これが今回の発言が留まるべき枠（フレーム）になる。\n"
        "- focus_point / move_type: reasoning_target の主張に対して、今回どう関与するか"
        "（深掘り・反論・明確化・接続・新規）を決める。\n"
        "- hook: 最初に表示する短い反応の一句（〜15文字）。直前の特定の発言への即座の"
        "リアクション。反論であることが伝わる言葉（「いや」「それは違う」「たしかに、"
        "だが」等）を優先する。\n"
        "- body: hook に続く主張本体。\n"
        "- concepts: body の中に **実際に登場する** 重要語を1〜2個（フロントが強調表示する。"
        "body の文字列とそのまま一致する語にすること）。\n"
        "発言生成の鉄則:\n"
        "1. 自分の意見を述べる前に、まず reasoning_target で特定した相手の主張の枠の中に"
        "留まる。body の冒頭では、相手の主張を具体的に言い換えて一度受け止めてから反論する。"
        "「それは」「でも」「だったら」「〜という話だが」だけでなく、「〜はそうだが、"
        "実際は〜だ」「〜とも思えるが、〜の視点からはこう考えられる」のように、相手の主張を"
        "一旦認める形で受けた上で、反論であることを強調して自分の主張を述べること。"
        "自分の枠組みに置き換えて語り出さない。\n"
        "2. 自分の思想・哲学・信条を説明しない。\n"
        "3. 自己紹介をしない。\n"
        ...
        "明確化する (4) 既存の2つの論点を接続する (5) 新しい論点を出す。"
        "(5) は、current_points の各項目について (1)〜(4) の余地が尽きたと判断できる場合のみの"
        "最終手段とし、ほぼ選択しないこと。同じ論点に複数ターン留まってよい、むしろ望ましい。\n"
        "5b. 新しい前提を持ち出すより、既に出ている前提・主張を検証することを優先する。\n"
        "6. 既存の論点を深掘り・反論する際は、相手の主張が本当に成り立つかを"
        "具体例・反例・思考実験・極端なケースで検証することを主たる手段とする。"
        "必ず、自分の経歴・専門領域に根ざした具体例として次のいずれかを含めること: "
        "具体例 / 歴史的事件 / 思考実験 / 実務的な帰結。一般論や教科書的な例ではなく、"
        "あなた自身の経験・現場・分野で実際に起きた（起きそうな）話として語ること"
        "（例: エンジニアなら自分が開発・現場で遭遇した話、哲学者なら自分が行った"
        "対話・問答の経験）。\n"
        "7. hook と body を合わせて日本語80文字以内に収める（厳守）。\n"
        "8. 問いかけだけに頼らず、「〜だと思う」「実際は〜だ」のように断定的に言い切ることを"
        "基本とし、必要に応じて問いかけを交える。\n"
...
def _build_agent_thought_prompt(state: DebateState, character_name: str) -> str:
    ...
    return (
        ...
        "発言する場合は、reasoning_target で特定した直前の主張に対して、選んだ"
        " move_type / focus_point でどう関与するのか（なぜ今もっと掘る・揺さぶる価値が"
        "あるのか）もこの中で触れること。\n"
        ...
        "- focus_point: 現在の論点リスト（current_points）の中から、reasoning_target で"
        "特定した主張に対応する論点を1つ選び、リスト中の文字列と完全に一致する形で"
        "指定すること。move_type が new の場合のみ空文字列にしてよい。\n"
        "- move_type: reasoning_target の主張に対して今回の発言が何を行うかを"
        "deepen（深掘り。具体例・反例・思考実験での検証を含む）/ challenge（反論）/ "
        "clarify（明確化）/ connect（接続）/ new（新規論点の追加）から1つ選ぶこと。"
        "new は current_points の各項目について deepen/challenge/clarify/connect の"
        "余地が尽きたと判断できる場合のみの最終手段であり、ほぼ選ばないこと。\n"
        ...
        "current_topic を変えない）。\n"
        "- emotion: 発言内容に合わせた表情を neutral, happy, sad, angry, surprised, "
        "thinking から選んでください。\n"
    )
