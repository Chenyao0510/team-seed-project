...
2. Frontend が各メンバー名について POST /api/add_character を呼び、アバター URL と persona を取得
   ...
   - 人物像・口調・専門・価値観を1〜2文にした `persona` を best-effort 生成 (T62 / D17)。失敗時は `""`
...
5. Backend が新しい active_character / 発言 (hook+body) / current_points / chat_history / turn_count を更新（characters は不変、追加は `/api/add_character` 経由のみ。turn_count は呼び出しごとに+1）
   - 発言は `current_hook` / `current_body` / `current_reasoning_target` / `current_concepts` の構造化形式 (D18)。`current_speech` は `hook + body` の合成導出値で TTS / archive 用
6. Backend が更新後 Debate State JSON を返却
7. Frontend が hook を即時表示 → 300〜600ms 後に body をタイプライターで描画、concepts を強調、reasoning_target を返信チップ表示 (D18)
```
...
| POST    | `/api/add_character` | `{"name": "string"}`  | `{"avatar_url": "string", "persona": "string"}`|
| GET     | `/api/character_templates` | -               | `[{slug, name, avatar_url, persona}, ...]` (T5A / D16 / D17) |
| POST    | `/api/next_turn`     | Debate State          | 更新後 Debate State        |
| POST    | `/api/summarize`     | Debate State          | Integration State          |
| GET     | `/api/tts`           | `text`, `character_name`| `audio/wav` バイナリ       |
