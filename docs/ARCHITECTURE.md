...
| 画像生成      | 画像生成 API (Nanobanana 等) + 背景透過処理                  |
...
+----------------------------------------------------+
       ...
       ▼                              ▼
+----------------+    +----------------------------+
|   Gemini API   |    |  画像生成 API (Nanobanana)  |
| (JSON Mode /   |    |  + 背景透過処理              |
|  Search        |    |                            |
|  Grounding)    |    |                            |
+----------------+    +----------------------------+
...
- 画像生成パイプラインは `app/avatar_pipeline.py` のような単一モジュールに閉じる
...
- 単一モジュールに閉じる。タイムアウト・リトライ・フェイルセーフを必ず実装
- 失敗時はプレースホルダーアバターを返し、UI を壊さない
- API キーは `.env` から読み込む
...
2. Frontend が各メンバー名について POST /api/add_character を呼び、アバター URL を取得
   - Gemini Search Grounding で人物のビジュアルリファレンスを取得
   - 画像生成 API でアバターを生成
   - 背景透過処理を実行
...
5. Backend が新しい active_character / current_speech / current_points / chat_history を更新（characters は不変、追加は `/api/add_character` 経由のみ）
...
3. Backend が Screen 0 と同じパイプライン（Gemini Search → 画像生成 → 透過処理）を実行
