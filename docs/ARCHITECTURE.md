...
| POST    | `/api/add_character` | `{"name": "string"}`  | `{"avatar_url": "string", "gender": "male\|female\|robot"}` (T69) |
| GET     | `/api/character_templates` | -               | `[{slug, name, avatar_url, gender}, ...]` (T5A / D16, T69) |
| POST    | `/api/next_turn`     | Debate State          | 更新後 Debate State        |
| POST    | `/api/summarize`     | Debate State          | Integration State          |
| GET     | `/api/tts`           | `text`, `character_name`, `gender?` (T69) | `audio/wav` バイナリ |
